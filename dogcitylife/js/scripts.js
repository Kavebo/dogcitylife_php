...
function vyhledavani_ajax(page){
	...
	$.get(window.location.pathname + "?" + data + '&page=' + page, function (response) {
		num = $(response).find(".search_line .results").html();
		$('.search_line .results').html(num);
	    response = $(response).find('#search_cont').html();
	    $("#search_cont").empty().append(response);
	    /*if($(window).width() <= 890){
...
$(document).ready(function(){
	...
	$(".sidebar .rating_input .rating .point").click(function(e){
		e.preventDefault();
		$(this).parent().removeClass("rating0 rating1 rating2 rating3 rating4 rating5");
		$(this).parent().addClass("rating" + $(this).attr("data-val"));
	...
	$(".sidebar .benefity .select_box").click(function(e){
		...
		$(this).parent().children("select").children('option[value="' + $(this).attr("data-val") + '"]').attr("selected", sel);
	...
	$("#list_review").on("click", ".pagination a", function(e){
		e.preventDefault();
		$(".loader").show();
		$('#list_review').load($(this).attr("href") + "&order=" + $("select#order").val() + ' #list_review > *', function(){
			$(".loader").hide();
		});
	});
	...
	$(".recenze").on("change", "select#order", function(e){
		...
		if($(this).hasClass("profile_review")){
			$('#list_review').load(window.location.pathname + window.location.search + '&order=' + $(this).val() + ' #list_review > *', function(){
				$(".loader").hide();
			});
		}else{
	...
	$(".top_header .user_header").hover(function(e){
		$(this).children(".dropdown_menu").stop(true,false).fadeToggle(300);
	});

	$(".top_header .mobile_menu .hamburger").click(function(e){
		e.preventDefault();
		$(this).next().stop(true,false).fadeToggle(300);
	...
	$( "form.review_form" ).validate({
	   ...
	   errorPlacement: function(error, element) {
		...
		submitHandler: function(form) {
			$(".loader").show();
		    //ajax login
		    var data = {zarizeni_ID: jQuery("#zarizeni_ID").val(), popis: jQuery("#review_text").val(), obsluha: jQuery("#obsluha").val(), dog_friendly: jQuery("#dog_friendly").val(), jidlo: jQuery("#jidlo").val(), prostredi: jQuery("#prostredi").val()};
